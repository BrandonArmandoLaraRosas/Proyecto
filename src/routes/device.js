...
deviceRouter.post('/actualizarDatos', async (req, res) => {
    try {
        ...
        if (huella !== undefined) {
            const historicohuella = new Historico({ idDevice: existingDevice._id, variable: 'huella', valor: huella, fecha: new Date() });
            await historicohuella.save();
        }
        ...
        const updatedData = await Dispositivo.findOne({ _id: device_label }, { pir: 1, rfid: 1, servo: 1, huella:1, keypad: 1, estado: 1, _id: 0 });
